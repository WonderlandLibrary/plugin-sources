...
public class CommandCredits extends AbstractCommand {
    ...
    public void execute(User user, PacketReceiveEvent event, WrapperPlayClientChatMessage packet, String[] args) {
        if(args.length < 1){
            user.sendMessage("Usage: credits <player>");
            return;
        }
        ...
        if(args[0].equals("*")){
            ...
            user.sendMessage("The credits screen has been displayed to all players.");
            return;
        }
        ...
        user.sendMessage("The credits screen has been displayed to " + user.getMainColor() + target.getName() + user.getSecondaryColor() + ".");
