package com.elevatemc.anticheat.base.gui.impl.check.misc.impl;
...
public class BackTrackGUI implements IGUI
{

    @Override
    public void openToPlayer(Player player) {
        SGMenu menu = HoodPlugin.INSTANCE.getGuiManager2().create(CC.PRI + "Back Track Checks", 5);

        for (String check : HoodPlugin.INSTANCE.getCheckManager().getAlphabeticallySortedChecks()) {
            if (!check.contains("BackTrack")) {
                continue;
            }
