package com.elevatemc.anticheat.base.gui.impl.check.movement.impl;
...
public class DisablerGUI implements IGUI {
    @Override
    public void openToPlayer(Player player) {
        SGMenu menu = HoodPlugin.INSTANCE.getGuiManager2().create(CC.PRI + "Disabler Checks", 5);

        for (String check : HoodPlugin.INSTANCE.getCheckManager().getAlphabeticallySortedChecks()) {
            if (!check.contains("Disabler")) {
                continue;
            }
