...
public class MathUtil {
    ...
    public static double getMean(final Collection<? extends Number> samples) {
        double sum = 0.0;
    ...
    public static double getExponentiallyWeightedMovingAverage(double EWMA_ALPHA, double currentSample, double previousEWMA) {
        return EWMA_ALPHA * currentSample + (1 - EWMA_ALPHA) * previousEWMA;
    }
    ...
    public static <T extends Number> double getDuplicates(final Collection<T> entry) {
        final double distinct = (double)entry.stream().distinct().count();
