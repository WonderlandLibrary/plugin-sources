...
public class PlayerChecks extends VulcanGUI
{
    public static final PlayerChecks instance;
    private final List<Class> playerChecks;
    private final String inventoryName = "Manage Player Checks";
    ...
    public PlayerChecks() {
        this.playerChecks = new ArrayList<Class>();
    ...
    public void initializeInventory() {
        ...
        this.inventory = this.createInventory(54, "Manage Player Checks");
        final List<Class> classList = new ArrayList<Class>(Arrays.asList(CheckManager.CHECKS));
        for (final Class clazz : classList) {
            if (clazz.getName().contains("player")) {
                this.playerChecks.add(clazz);
            }
        }
        this.maxPages = 0;
        this.maxPages = (int)Math.ceil(this.playerChecks.size() / 45.0);
        int offset = 0;
        int currentPage = 1;
        for (int i = 0; i <= this.maxPages; ++i) {
            final Inventory temporaryInventory = this.createInventory(54, "Manage Player Checks [" + currentPage + "/" + this.maxPages + "]");
            ...
            for (int checkSlot = 0; checkSlot < 45; ++checkSlot) {
                if (offset <= this.playerChecks.size() - 1) {
                    final String checkClassName = this.playerChecks.get(offset).getSimpleName();
                    final CheckInfo checkInfo = (CheckInfo)this.playerChecks.get(offset).getAnnotation(CheckInfo.class);
                    ...
                    final String path = "checks.player." + checkNameToLowercase + "." + checkTypeToLowercase + ".";
    ...
    public List<Class> getPlayerChecks() {
        return this.playerChecks;
    }
    
    public String getInventoryName() {
        Objects.requireNonNull(this);
        return "Manage Player Checks";
    }
    ...
    public static PlayerChecks getInstance() {
        return PlayerChecks.instance;
    }
    
    static {
        instance = new PlayerChecks();
    }
}
